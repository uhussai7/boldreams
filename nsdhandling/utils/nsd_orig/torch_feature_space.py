...
class Torch_filter_fmaps(nn.Module):
    ...
    def forward(self, _x):
        ...
        return [torch.index_select(torch.cat([_fmaps[l] for l in lm], axis=1), dim=1, index=fm) for lm, fm in
                zip(self.lmask, self.fmask)]
...
def filter_dnn_feature_maps(data, _fmaps_fn, batch_size, fmap_max=1024, concatenate=True, trn_size=None):
    ...
    #_fmaps_fn is alexnet but nicely labelled. fmaps_fn is putting _fm to cpu
    fmaps = fmaps_fn(data[:batch_size]) #just passing the data/inputs through alexnet to get sizes
    run_avg = [np.zeros(shape=(fm.shape[1]), dtype=np.float64) for fm in fmaps] #size of both is [nlayers][nchannels]
    run_sqr = [np.zeros(shape=(fm.shape[1]), dtype=np.float64) for fm in fmaps]
    for rr, rl in tqdm(iterate_range(0, size, batch_size)): #now pass all data through fmaps
        fb = fmaps_fn(data[rr]) #this fmaps for a batch
        for k, f in enumerate(fb): #going over each layer
            if f.shape[
                1] > fmap_max:  # only need the average if we're going to use them to reduce the number of feature maps
                run_avg[k] += np.sum(np.mean(f.astype(np.float64), axis=(2, 3)), axis=0) #mean over pixels
                run_sqr[k] += np.sum(np.mean(np.square(f.astype(np.float64)), axis=(2, 3)), axis=0)#squared mean pixels
    for k in range(len(fb)): #going from sum above to average
        run_avg[k] /= size
        run_sqr[k] /= size
    ###
    fmask = [np.zeros(shape=(fm.shape[1]), dtype=bool) for fm in fmaps] #just bool of size [nlayers][nchannels]
    fmap_var = [np.zeros(shape=(fm.shape[1]), dtype=np.float32) for fm in fmaps] #same but float
    most_var_list=[]
    for k, fm in enumerate(fmaps):
        if fm.shape[1] > fmap_max:
            # select the feature map with the most variance to the dataset
            fmap_var[k] = (run_sqr[k] - np.square(run_avg[k])).astype(np.float32) #computes var of all channels in layer
            most_var = fmap_var[k].argsort()[-fmap_max:]  # the feature indices with the top-fmap_max variance
            fmaps[k] = fm[:, np.sort(most_var), :, :] #filters out the most variance fmaps
            fmask[k][most_var] = True #this is what we need, what happens to this?
            most_var_list.append(most_var)
        else: #this is if we want all filters becuase less than equal to fmap_max
            fmask[k][:] = True
            most_var_list.append(np.arange(0,len(fmask[k])))
        print("layer: %s, shape=%s" % (k, (fmaps[k].shape)))
        sys.stdout.flush()

    # ORIGINAL PARTITIONING OF LAYERS
    fmaps_sizes = [fm.shape for fm in fmaps]
    fmaps_count = sum([fm[1] for fm in fmaps_sizes]) #total filters
    partitions = [0, ]
    for r in fmaps_sizes:
        partitions += [partitions[-1] + r[1], ]
    layer_rlist = [range(start, stop) for start, stop in
                   zip(partitions[:-1], partitions[1:])]  # the frequency ranges list
    ...
    if concatenate:
        for k, us in enumerate(np.unique([np.prod(fs[2:4]) for fs in fmaps_sizes])[
            ...
            mask = np.array(
                [np.prod(fs[2:4]) == us for fs in fmaps_sizes])  # mask over layers that have that spatial size
