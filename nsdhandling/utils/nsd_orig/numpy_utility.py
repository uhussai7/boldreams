...
def gaussian_mass(xi, yi, dx, dy, x, y, sigma):
    return 0.25 * (erf((xi - x + dx / 2) / (np.sqrt(2) * sigma)) - erf((xi - x - dx / 2) / (np.sqrt(2) * sigma))) * (
                erf((yi - y + dy / 2) / (np.sqrt(2) * sigma)) - erf((yi - y - dy / 2) / (np.sqrt(2) * sigma)))
...
def pruning_mask(shaped_as, prune_ratio=0.0):
    ...
    return np.random.choice([True, False], size=shaped_as.shape[1], replace=True,
                            p=[1. - prune_ratio, prune_ratio])  # astype(np.bool)
